package com.commandiron.besonapp_clean_arch.presentation.signup_steps.as_customer
...
fun SignUpStepsAsCustomerScreen1(
        ...
        viewModel.uiEvent.collect{ event ->
            when(event) {
                ...
                is SignUpStepsUiEvent.NavigateToNextScreen -> {
                    navController.navigate(
                        NavigationItem.SignUpStepsAsCustomer2.route
                    )
                }
                else -> {}
            }
    ...
    Column(
        ...
        RegistrationTextField(
            text = state.name,
            hint = YOUR_NAME,
            onChange = { viewModel.onEvent(SignUpStepsUserEvent.NameChanged(it)) }
        )
        Spacer(modifier = Modifier.height(spacing.spaceLarge))
        Button(onClick = { viewModel.onEvent(SignUpStepsUserEvent.NameScreenNext) }) {
            Text(text = NEXT)
        }
    }
    Box(
