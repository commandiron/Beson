...
fun SignUpStepsAsCompanyScreen4(
        ...
        viewModel.uiEvent.collect{ event ->
            when(event) {
                ...
                is SignUpStepsUiEvent.NavigateToNextScreen -> {
                    navController.navigate(
                        NavigationItem.SignUpStepsAsCompany5.route
                    )
                }
                else -> {}
            }
    ...
    Column(
        ...
        SingleCategorySelector(
            ...
            onItemSelected = {
                viewModel.onEvent(SignUpStepsUserEvent.MainCategorySelected(it))
            }
        )
        Spacer(modifier = Modifier.height(spacing.spaceLarge))
        Button(
            onClick = { viewModel.onEvent(SignUpStepsUserEvent.ConstructionCategoryScreenNext) }
        ) {
            Text(text = NEXT)
        }
    }
